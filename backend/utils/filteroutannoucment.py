...
def is_announcement(title: str) -> bool:
    ...
    religious_keywords = ['navratri', 'diwali', 'holi', 'eid', 'christmas', 'festival', 'goddess', 'god', 'prayer', 'blessing']
    for keyword in religious_keywords:
        if keyword in title_lower:
            return True
    
   
    social_media_patterns = [r'shares.*on.*social media', r'tweets.*about', r'posts.*video']
...
def filter_announcements(press_releases, filter_type="exclude"):
    if filter_type == "exclude":
        return [pr for pr in press_releases if not is_announcement(pr['title'])]
    else:
        return [pr for pr in press_releases if is_announcement(pr['title'])]
