...
const KeyInforMation: React.FC<Props> = ({ heading, points }) => {
    return (
        <div className="flex flex-col gap-4 font-satoshi">
            ...
            {/* Points */}
            <ul className="flex flex-col gap-3 pl-[15px] border-l border-[#ff3333]/10">
                {points.map((point, index) => (
                    <li key={index} className="flex items-start gap-3 w-full md:w-[75%]">
                        <span className="mt-[6px] w-[6px] h-[6px] rounded-full bg-[#ff3333]/50 flex-shrink-0" />
                        <p className="text-[#321F1F]/60 text-[1rem] md:text-[1.1rem] leading-7">
                            {point}
                        </p>
                    </li>
                ))}
